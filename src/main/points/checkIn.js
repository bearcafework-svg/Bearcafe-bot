...
function setupCheckIn(client) {
  ...
  client.on('messageCreate', async (message) => {
    ...
    // ── ตรวจสอบ Cooldown ──────────────────────────────────────────────────────
    const has1h = member.roles.cache.has(cfg.roles.premium_1h);
    ...
    if (has1h) cdDuration = cfg.cooldown_ms.premium_1h;
    else if (has2h) cdDuration = cfg.cooldown_ms.premium_2h;
    ...
    if (has1h) {
      premiumRoleInfo = {
        name: member.guild.roles.cache.get(cfg.roles.premium_1h)?.name || 'Premium',
        emoji: '<:Limited_26:1542162263253848164>'
      };
    } else if (has2h) {
      premiumRoleInfo = {
    ...
    await message.reply(claimPayload).catch(() => message.channel.send(claimPayload).catch(() => {}));
  });
