...
async function safeDeferUpdate(interaction) {
  ...
  try {
    ...
    if (err.code !== 40060 && err.code !== 10062) {
      console.error("[donate] deferUpdate error:", err);
    }
    return false;
  }
}
...
async function buildTopDonateComponents(guild, supabase) {
  ...
  const components = [
    ...
    // Header + ปุ่มเช็คยอด
    {
      ...
      components: [{
        type: 10,
        content: "## <a:bellbag:1185624186132103178>︲__` 𝖳𝗈𝗉 𝖣𝗈𝗇𝖺𝗍𝖾 ₊ ท็อปโดเนท 𓂃 `__ \n-# <:line:1144701793989840997> มาดูกันว่าใครสายเปย์ที่สุดในคาเฟ่หมี หากยอดโดเนทของคุณครบ **1,000 บาท** ขึ้นไปก็รับยศ <@&1185550863448682579> ผ่านห้อง <#1524124172580814979> ได้เลยค่ะ <:cuteplant:1152834055528783872>"
      }],
      accessory: {
        style: 1,
        type: 2,
...
function setupDonate(client) {
  ...
  client.on("interactionCreate", async (interaction) => {
    ...
    if (interaction.customId === "donate_check_self") {
      ...
      try {
        if (!(await safeDeferReply(interaction, { flags: 64 }))) return;
        ...
        return safeRespond(interaction, { content: "❌ เกิดข้อผิดพลาดค่ะ กรุณาลองใหม่อีกครั้ง" });
      }
    }
  });
}
