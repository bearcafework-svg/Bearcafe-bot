...
/**
 * บันทึกรูปภาพแผงควบคุมบ้านเช่า และ Auto-Sync ไปยังห้อง VIP ของ owner_id เดียวกัน
 */
async function saveRentHouseImage(channelId, ownerId, imageUrl) {
  const supabase = getSupabase();
  if (supabase) {
    ...
    const newSetting = {
      ...
      trusted_user_ids: current?.trusted_user_ids || [],
      image_url: imageUrl,
      updated_at: new Date().toISOString(),
    };
    ...
    // Auto-sync ไปยังห้อง VIP (smart_room_presets) ของ owner เดียวกัน
    try {
      const { saveSmartRoomPreset, getSmartRoomPreset } = require("../../../../utils/smartRoomPresets");
      const currentPreset = (await getSmartRoomPreset(ownerId, "vip")) || {};
      await saveSmartRoomPreset(ownerId, "vip", {
        ...currentPreset,
        imageUrl: imageUrl,
      });
    } catch (err) {
      console.warn("[rentHouseService] Auto-sync to smart_room_presets failed:", err.message);
    }
  }
  return true;
}
