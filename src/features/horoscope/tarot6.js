...
const OTHER_COMMANDS_ID = 'tarot6_other_commands';
const MISSION_CLAIM_ID  = 'tarot6_mission_claim';
...
// ─── Helper: สุ่มค่า stat จาก [min, max] ─────────────────────────────────────
function randStat(range) {
...
async function getUserRow(supabase, userId) {
  ...
  if (error) {
    ...
    console.error('[tarot6] getUserRow error:', error.message);
...
async function addPoints(supabase, userId, pointsDelta, tarotPointDelta = 0) {
  ...
  if (error) {
    console.error('[tarot6] addPoints RPC error:', error.message);
...
function buildCardPayload(card, stats, earnedPoints) {
  const pi = cfg.point_icon;
  return {
    flags: FLAG_V2,
    components: [{
      ...
      components: [
        ...
        { type: 14, spacing: 2 },
        {
          ...
          components: [{
            type: 10,
            content:
              `## <:bee20000:1256669436350562355>︲__\` ${card.name} ₊ ${card.meaning} 𓂃 \`__\n` +
              `-# ${card.prediction} <:cuteplant:1152834055528783872>`
          }],
          accessory: {
            type:  2,
            style: 5,
...
function buildCombinedPayload(card, stats, earnedPoints, tarotPoint, isComplete) {
  const pi = cfg.point_icon;
  return {
    flags: FLAG_V2,
    components: [{
...
function setupTarot6(client) {
  ...
  // ── Listener: ข้อความ "รูนประจำตัว" ────────────────────────────────────
  client.on('messageCreate', async (message) => {
    if (!message.guild)     return;
    if (message.author.bot) return;
    if (message.channel.id !== cfg.channel_id) return;
    if (message.content.trim() !== 'รูนประจำตัว') return;
    ...
    const now        = Date.now();
    const cdExpiry   = await getCooldown(supabase, userId, 'tarot6');

    if (now < cdExpiry) {
      await message.reply({ content: cooldownContent(userId, Math.floor(cdExpiry / 1000)) });
      return;
    }
    await setCooldown(supabase, userId, 'tarot6', now + cdDuration);
    ...
    // ── สุ่มรูน (1–24) จาก Infotarot4.json + สุ่ม stats + แต้ม ─────────────
    const cardId       = String(randInt(1, 24));
    const card         = infotarot4.cards[cardId];
    const tpl          = infotarot4.statsTemplate;
    const stats = {
      love:  randStat(tpl.love),
      luck:  randStat(tpl.luck),
      money: randStat(tpl.money),
      charm: randStat(tpl.charm),
    };
    ...
    const deletePromise = loadingMsg.deletable
      ? loadingMsg.delete().catch(err => console.error('[tarot6] delete loading error:', err.message))
      : Promise.resolve();

    const sendPromise = alreadyClaimed
      ? message.reply(buildCardPayload(card, stats, earnedPoints))
      : message.reply(buildCombinedPayload(card, stats, earnedPoints, newTarotPoint, missionComplete));
    ...
    if (customId === MISSION_CLAIM_ID) {
      try {
        ...
        try {
          ...
          console.error('[tarot6] addRole error:', err.message);
        }
        ...
        if (error.code !== 40060 && error.code !== 10062) {
          console.error('[tarot6] Mission Claim Error:', error);
        }
      }
    }
  });

  console.log('[tarot6] ✅ ระบบดูดวง tarot6 พร้อมใช้งาน');
}

module.exports = { setupTarot6 };
