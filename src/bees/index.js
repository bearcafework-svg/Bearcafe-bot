      ...
      if (guild) {
        ...
        await guild.commands.create({
          ...
          options: [
            ...
            {
              name: 'state',
              description: 'กรณีเลือกพรีวิว: เลือกระบุสถานะที่ต้องการดูแบบเฉพาะเจาะจง (เว้นว่างเพื่อดูทั้งหมด)',
              type: 3, // String
              required: false,
              choices: [
                { name: '🌟 เกิดใหม่ (Spawn)', value: 'spawn' },
                { name: '🎉 ชนะ/ขโมยสำเร็จ (Win/Success)', value: 'win' },
                { name: '👑 แจ็กพอตมงกุฎ (Queen Crown)', value: 'crown' },
                { name: '💢 โดนต่อย/แพ้ (Loss/Attack)', value: 'loss' },
                { name: '☠️ ติดพิษ/ล้มละลาย (Poison/Bankrupt)', value: 'poison' },
                { name: '🦇 แวมไพร์ตื่น 60 วิ (Awaken)', value: 'awaken' },
                { name: '🎯 แวมไพร์ดูดแต้มเหยื่อ (Vampire Drain)', value: 'vampire_drain' },
                { name: '🕵️ สายลับแจกแต้ม/ลดแต้ม/มีม/เควสต์ (Spy Rewards)', value: 'spy_rewards' },
                { name: '🐷 เควสต์หมู อู๊ดอู๊ด (Spy Quest)', value: 'quest' },
                { name: '🚪 ผึ้งบินกลับรัง (Expired 15 นาที)', value: 'expired' }
              ]
            }
          ]
        });

        console.log('[bees] Commands /spawn_bee, /bee_config & /test_bee registered successfully.');
      }
    } catch (err) {
